...
pub async fn handle_follow(
    ...
    match (j.get("@context").and_then(|v| v.as_str()), 
        ...
        (Some(ctx), Some(id), Some(actor), Some(object)) => {
            if ctx != "https://www.w3.org/ns/activitystreams" {
                return Err(anyhow::anyhow!("unkonwn message type"));
            }
        ...
        (_, _, _, _) => Err(anyhow::anyhow!("failed to parse body"))
    }
}
    ...
    match (j.get("@context").and_then(|v| v.as_str()), 
        ...
        (Some(ctx), Some(_id), Some(actor), Some(object)) => {
            ...
            match util::prepare_message(req, "/", payload) {
                Ok(msg) => {
                    dispatcher.send(msg).await
                }
                Err(e) => {
                    Err(anyhow::anyhow!("Failed to prepare message: {}", e))
                }
            }
        }
        (_, _, _, _) => Err(anyhow::anyhow!("failed to parse body"))
